      ...
      {!isAdmin && (
        <div className="mt-2.5 pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
          {profileHref ? (
            ...
            >
              <span>{isOwnStrategy ? 'Strategy Analytics' : 'Profile'}</span>
          ...
          {isOwnStrategy ? (
            <span className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-500/15 via-amber-500/10 to-yellow-500/20 border border-amber-300 text-amber-900 font-extrabold text-[11px] flex items-center gap-1.5 shadow-2xs font-heading">
              <span className="text-amber-600">👑</span>
              <span>Your Strategy (Live)</span>
            </span>
          ) : isBeingCopied ? (
